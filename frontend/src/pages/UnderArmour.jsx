...
// Export Under Armour products array for use in other components
let underArmourProducts = [];
...
const UnderArmour = () => {
  ...
  // Fetch Under Armour products from Supabase
  const fetchUnderArmourProducts = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from('underarmour_products')
        .select('*')
        .eq('is_active', true)
      ...
      const transformedProducts = data.map(product => ({
        id: product.id,
        uniqueId: `underarmour-${product.id}`,
        title: product.title,
      ...
      underArmourProducts = transformedProducts;
      setProducts(transformedProducts);
    } catch (error) {
      console.error('Error fetching Under Armour products:', error);
      underArmourProducts = [];
      setProducts([]);
    } finally {
      setLoading(false);
  ...
  // Fetch Under Armour jackets from jackets table
  const fetchUnderArmourJackets = async () => {
    try {
      ...
      console.error('Error fetching Under Armour jackets:', error);
      setJackets([]);
    } finally {
  ...
  useEffect(() => {
    fetchUnderArmourProducts();
    fetchUnderArmourJackets();
  }, []);
  ...
  const handleJacketClick = () => {
    navigate('/jackets?brand=Under Armour');
  };
      ...
      <div className="min-h-screen bg-white pt-8 pb-24 md:pb-8">
        <div className="max-w-[90%] lg:max-w-[80%] mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-black mb-4">Under Armour</h1>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Protect This House. Discover the latest Under Armour performance footwear 
              featuring innovative technologies, superior comfort, and athletic excellence for every workout.
            </p>
          </div>
          ...
          {loading && (
            <div className="text-center py-12">
              <p className="text-gray-600">Loading Under Armour products...</p>
            </div>
          )}
          ...
          {/* Under Armour Jackets Section with Marquee Animation - SAME AS OTHER BRANDS */}
          {!jacketsLoading && jackets.length > 0 && (
            <div className="border-t border-gray-200 pt-12 pb-8 mt-12">
              {/* Centered Heading */}
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-black mb-2">Under Armour Jackets</h2>
                <p className="text-gray-600 text-sm">
                  Explore our premium Under Armour jacket collection
                </p>
              </div>
