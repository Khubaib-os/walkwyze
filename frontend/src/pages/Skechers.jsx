...
// Export Skechers products array for use in other components
let skechersProducts = [];
...
const Skechers = () => {
  ...
  // Fetch Skechers products from Supabase
  const fetchSkechersProducts = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from('skechers_products')
        .select('*')
        .eq('is_active', true)
      ...
      skechersProducts = transformedProducts;
      setProducts(transformedProducts);
    } catch (error) {
      console.error('Error fetching Skechers products:', error);
      skechersProducts = [];
      setProducts([]);
    } finally {
      setLoading(false);
  ...
  // Fetch Skechers jackets from jackets table
  const fetchSkechersJackets = async () => {
    try {
      ...
      console.error('Error fetching Skechers jackets:', error);
      setJackets([]);
    } finally {
  ...
  useEffect(() => {
    fetchSkechersProducts();
    fetchSkechersJackets();
  }, []);
  ...
  const handleJacketClick = () => {
    navigate('/jackets?brand=Skechers');
  };
      ...
      <div className="min-h-screen bg-white pt-8 pb-24 md:pb-8">
        <div className="max-w-[90%] lg:max-w-[80%] mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-black mb-4">Skechers</h1>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Comfort is everything. Discover the latest Skechers footwear collection featuring 
              innovative comfort technologies, stylish designs, and all-day support for every step.
            </p>
          </div>
          ...
          {loading && (
            <div className="text-center py-12">
              <p className="text-gray-600">Loading Skechers products...</p>
            </div>
          )}
          ...
          {/* Skechers Jackets Section with Marquee Animation - SAME AS OTHER BRANDS */}
          {!jacketsLoading && jackets.length > 0 && (
            <div className="border-t border-gray-200 pt-12 pb-8 mt-12">
              {/* Centered Heading */}
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-black mb-2">Skechers Jackets</h2>
                <p className="text-gray-600 text-sm">
                  Explore our premium Skechers jacket collection
                </p>
              </div>
