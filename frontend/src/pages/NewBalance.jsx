...
// Export New Balance products array for use in other components
let newBalanceProducts = [];
...
const NewBalance = () => {
  ...
  // Fetch New Balance products from Supabase
  const fetchNewBalanceProducts = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from('newbalance_products')
        .select('*')
        .eq('is_active', true)
      ...
      newBalanceProducts = transformedProducts;
  ...
  // Fetch New Balance jackets from jackets table
  const fetchNewBalanceJackets = async () => {
    try {
      ...
      console.error('Error fetching New Balance jackets:', error);
      setJackets([]);
    } finally {
  ...
  const handleJacketClick = () => {
    navigate('/jackets?brand=New Balance');
  };
      ...
      <div className="min-h-screen bg-white pt-8 pb-24 md:pb-8">
        <div className="max-w-[90%] lg:max-w-[80%] mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-black mb-4">New Balance</h1>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Fearlessly Independent Since 1906. Discover the latest New Balance footwear collection 
              featuring superior craftsmanship, innovative technologies, and timeless style for every step.
            </p>
          </div>
          ...
          {loading && (
            <div className="text-center py-12">
              <p className="text-gray-600">Loading New Balance products...</p>
            </div>
          )}
          ...
          {/* New Balance Jackets Section with Marquee Animation - SAME AS OTHER BRANDS */}
          {!jacketsLoading && jackets.length > 0 && (
            <div className="border-t border-gray-200 pt-12 pb-8 mt-12">
              {/* Centered Heading */}
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-black mb-2">New Balance Jackets</h2>
                <p className="text-gray-600 text-sm">
                  Explore our premium New Balance jacket collection
                </p>
              </div>
