...
const PantsMgt = () => {
  ...
  // Pants sizes (waist)
  const sizeOptions = ["22", "24", "26", "28", "30", "32", "34", "36", "38", "40"];
  ...
  const fetchPants = async () => {
    ...
    try {
      ...
      showToast('Error fetching pants: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        if (error) throw error;
        showToast('Pants updated successfully!', 'success');
      } else {
        const { error } = await supabase
          .from('pants_products')
          .insert([productData]);
        if (error) throw error;
        showToast('Pants added successfully!', 'success');
      }
      resetForm();
      fetchPants();
    } catch (error) {
      showToast('Error saving pants: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete these pants?')) return;
    try {
      const { error } = await supabase
        .from('pants_products')
        .delete()
        .eq('id', id);
      if (error) throw error;
      showToast('Pants deleted successfully!', 'success');
      fetchPants();
    } catch (error) {
      showToast('Error deleting pants: ' + error.message, 'error');
    }
  };
  ...
  const toggleProductStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Pants ${!product.is_active ? 'activated' : 'deactivated'}!`, 'success');
      fetchPants();
    } catch (error) {
  ...
  const toggleSoldOutStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Pants marked as ${!product.is_sold_out ? 'Sold Out' : 'Available'}!`, 'success');
      fetchPants();
    } catch (error) {
  ...
  return (
    <div className="p-4 mt-2 lg:mt-8 relative">
      <h2 className="text-xl font-bold text-black mb-4">Pants Management</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
        <h3 className="text-base font-semibold mb-3">
          {editingProduct ? 'Edit Pants' : 'Add New Pants'}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Size (Waist) *</label>
                <select
                  name="size"
                  value={formData.size}
                  onChange={handleInputChange}
          ...
          <div className="space-y-3">
            ...
            <div className="mt-3">
              <label className="block text-xs font-medium text-gray-700 mb-2">
                Upload Pants Images (Max 10MB each) *
              </label>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                <input
                  ...
                  className="hidden"
                  id="image-upload-pants"
                  disabled={uploading}
                />
                <label
                  htmlFor="image-upload-pants"
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows="3"
              className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-transparent"
              placeholder="Enter pants description..."
            />
          </div>
          ...
          <div className="md:col-span-2 flex flex-wrap gap-3 pt-4 border-t">
            ...
            >
              {loading ? 'Saving...' : (editingProduct ? 'Update Pants' : 'Add Pants')}
            </button>
            {editingProduct && (
              <button
                type="button"
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-base font-semibold">Pants List ({products.length})</h3>
          <button
            onClick={fetchPants}
        ...
        {loading ? (
          <p className="text-gray-600 text-sm text-center py-8">Loading pants...</p>
        ) : products.length === 0 ? (
          <p className="text-gray-600 text-sm text-center py-8">No pants found. Add your first pants above.</p>
        ) : (
          <>
            {/* Desktop Table */}
            <div className="hidden md:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Pants</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-sm">Waist {product.size}</td>
                ...
                <div key={product.id} className="border rounded-lg p-4 bg-white shadow-sm">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">Waist {product.size}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{product.gender}'s</p>
                    </div>
                    <div>
