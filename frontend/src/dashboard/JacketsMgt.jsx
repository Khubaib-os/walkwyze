...
const JacketsMgt = () => {
  ...
  const brandOptions = ["Nike", "Adidas", "Puma", "New Balance", "Hoka", "Fila", "Under Armour", "Skechers"];
  ...
  const fetchJackets = async () => {
    ...
    try {
      ...
      showToast('Error fetching jackets: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.title || !formData.original_price || !formData.brand || !formData.height || !formData.width) {
      ...
      if (editingProduct) {
        ...
        if (error) throw error;
        showToast('Jacket updated successfully!', 'success');
      } else {
        const { error } = await supabase
          .from('jackets_products')
          .insert([productData]);
        if (error) throw error;
        showToast('Jacket added successfully!', 'success');
      }
      resetForm();
      fetchJackets();
    } catch (error) {
      showToast('Error saving jacket: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this jacket?')) return;
    try {
      const { error } = await supabase
        .from('jackets_products')
        .delete()
        .eq('id', id);
      if (error) throw error;
      showToast('Jacket deleted successfully!', 'success');
      fetchJackets();
    } catch (error) {
      showToast('Error deleting jacket: ' + error.message, 'error');
    }
  };
  ...
  const toggleProductStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Jacket ${!product.is_active ? 'activated' : 'deactivated'}!`, 'success');
      fetchJackets();
    } catch (error) {
  ...
  const toggleSoldOutStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Jacket marked as ${!product.is_sold_out ? 'Sold Out' : 'Available'}!`, 'success');
      fetchJackets();
    } catch (error) {
  ...
  return (
    <div className="p-4 mt-2 lg:mt-8 relative">
      <h2 className="text-xl font-bold text-black mb-4">Jackets Management</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
        <h3 className="text-base font-semibold mb-3">
          {editingProduct ? 'Edit Jacket' : 'Add New Jacket'}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="mt-3">
              <label className="block text-xs font-medium text-gray-700 mb-2">
                Upload Jacket Images (Max 10MB each) *
              </label>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                <input
                  ...
                  className="hidden"
                  id="image-upload-jackets"
                  disabled={uploading}
                />
                <label
                  htmlFor="image-upload-jackets"
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows="3"
              className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-black focus:border-transparent"
              placeholder="Enter jacket description..."
            />
          </div>
          ...
          <div className="md:col-span-2 flex flex-wrap gap-3 pt-4 border-t">
            ...
            >
              {loading ? 'Saving...' : (editingProduct ? 'Update Jacket' : 'Add Jacket')}
            </button>
            {editingProduct && (
              <button
                type="button"
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-base font-semibold">Jackets List ({products.length})</h3>
          <button
            onClick={fetchJackets}
        ...
        {loading ? (
          <p className="text-gray-600 text-sm text-center py-8">Loading jackets...</p>
        ) : products.length === 0 ? (
          <p className="text-gray-600 text-sm text-center py-8">No jackets found. Add your first jacket above.</p>
        ) : (
          <>
            {/* Desktop Table */}
            <div className="hidden md:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Jacket</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Price</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Brand</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Gender</th>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Measurements</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-sm">{product.gender}'s</td>
                      <td className="px-3 py-3 text-sm">
                        <div className="text-xs">
                          <div>H: {product.height}</div>
                          <div>W: {product.width}</div>
                        </div>
                      </td>
                      <td className="px-3 py-3">
                        <button
                ...
                <div key={product.id} className="border rounded-lg p-4 bg-white shadow-sm">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 text-xs text-gray-600">
                        <span className="font-medium">{product.brand}</span> • {product.gender}'s
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-xs">H: {product.height}</p>
                      <p className="text-xs">W: {product.width}</p>
                    </div>
                  </div>
