...
const ShirtsMgt = () => {
  ...
  const fetchShirts = async () => {
    ...
    try {
      ...
      showToast('Error fetching shirts: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.title || !formData.original_price || !formData.size || !formData.height || !formData.width) {
      ...
      if (editingProduct) {
        ...
        if (error) throw error;
        showToast('Shirt updated successfully!', 'success');
      } else {
        const { error } = await supabase
          .from('shirts_products')
          .insert([productData]);
        if (error) throw error;
        showToast('Shirt added successfully!', 'success');
      }
      resetForm();
      fetchShirts();
    } catch (error) {
      showToast('Error saving shirt: ' + error.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this shirt?')) return;
    try {
      const { error } = await supabase
        .from('shirts_products')
        .delete()
        .eq('id', id);
      if (error) throw error;
      showToast('Shirt deleted successfully!', 'success');
      fetchShirts();
    } catch (error) {
      showToast('Error deleting shirt: ' + error.message, 'error');
    }
  };
  ...
  const toggleProductStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Shirt ${!product.is_active ? 'activated' : 'deactivated'}!`, 'success');
      fetchShirts();
    } catch (error) {
  ...
  const toggleSoldOutStatus = async (product) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', product.id);
      if (error) throw error;
      showToast(`Shirt marked as ${!product.is_sold_out ? 'Sold Out' : 'Available'}!`, 'success');
      fetchShirts();
    } catch (error) {
  ...
  return (
    <div className="p-4 mt-2 lg:mt-8 relative">
      <h2 className="text-xl font-bold text-black mb-4">Shirts Management</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
        <h3 className="text-base font-semibold mb-3">
          {editingProduct ? 'Edit Shirt' : 'Add New Shirt'}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="mt-3">
              <label className="block text-xs font-medium text-gray-700 mb-2">
                Upload Shirt Images (Max 10MB each) *
              </label>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                <input
                  ...
                  className="hidden"
                  id="image-upload-shirts"
                  disabled={uploading}
                />
                <label
                  htmlFor="image-upload-shirts"
          ...
          <div className="md:col-span-2 flex flex-wrap gap-3 pt-4 border-t">
            ...
            >
              {loading ? 'Saving...' : (editingProduct ? 'Update Shirt' : 'Add Shirt')}
            </button>
            {editingProduct && (
              <button
                type="button"
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-base font-semibold">Shirts List ({products.length})</h3>
          <button
            onClick={fetchShirts}
        ...
        {loading ? (
          <p className="text-gray-600 text-sm text-center py-8">Loading shirts...</p>
        ) : products.length === 0 ? (
          <p className="text-gray-600 text-sm text-center py-8">No shirts found. Add your first shirt above.</p>
        ) : (
          <>
            {/* Desktop Table */}
            <div className="hidden md:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium text-gray-500 uppercase">Shirt</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-sm">{product.gender}</td>
